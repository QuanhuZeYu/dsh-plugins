...
export const name = 'subagent-effort-in-process';
...
function applyRequestedEffort(ctx) {
    ctx.on('agent/request', async ({ agent }, next) => {
        ...
        if (requested === undefined || proposal.reasoningEffort !== undefined)
            return proposal;
        return { ...proposal, reasoningEffort: requested };
    });
}
...
function registerTools(ctx, providerName, toolName, listToolName) {
    ctx.tools.register(defineTool({
        name: toolName,
        description: 'Delegate a self-contained task to an in-process child agent with an explicit model and '
            + 'reasoning effort. The child works in its own context and returns its final answer; its '
            + 'intermediate steps stay out of this conversation. This call waits for the child and returns '
            + 'its result.',
        parameters: {
            description: {
                type: 'string',
            ...
            model: {
                type: 'string',
                description: 'Optional model id for the child agent, from `list_models`. Omitted means the child '
                    + 'inherits the parent conversation model. A value the routed provider does not serve fails '
                    + 'the child before any model request.',
            },
            reasoning_effort: {
                type: 'string',
                description: 'Optional reasoning effort for the child agent: `off`, `low`, `high`, or `max`. '
                    + 'Omitted means the child uses its routed model default; a value the routed model does not '
                    + 'support fails the child run with UNSUPPORTED_REASONING_EFFORT before any model request.',
            },
            run_in_background: {
                type: 'boolean',
                description: 'Whether to run as a background job and return its id. Defaults to false; collect '
                    + 'with job_output or stop with job_kill.',
            },
        },
        output: {
            schema: {
        ...
        async execute(args, exec) {
            ...
            const agentOptions = {
                ...
                ...(args.reasoning_effort === undefined
                    ? {}
                    : { reasoningEffort: ReasoningEffortId(args.reasoning_effort) }),
            };
            const request = {
            ...
            if (args.run_in_background === true) {
                ...
                if (jobs === undefined) {
                    throw new Error(`${toolName}: background jobs unavailable (load @deepseek-ai/dsh-jobs in the composition)`);
                }
    ...
    ctx.tools.register(defineTool({
        name: listToolName,
        description: 'List model ids available on one provider route (advisory: the route may accept unlisted '
            + 'ids). Use the ids for the `model` argument of the delegation tool.',
        parameters: {
            provider: {
                type: 'string',
        ...
        async execute(args, exec) {
            ...
            if (provider === undefined || provider.length === 0) {
                throw new Error(`${listToolName} needs a provider route`);
            }
