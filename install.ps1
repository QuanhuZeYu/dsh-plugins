...
Write-Host "[1/4] @deepseek-ai 目录: $aiDir" -ForegroundColor Green
...
foreach ($p in $plugins) {
    ...
    Write-Host "[2/4] 已安装插件包: $p" -ForegroundColor Green
}
...
if (-not (Test-Path $uiTarget)) {
    ...
    try {
        ...
        Write-Host '[3/4] 已建立 profiles 树 junction: dsh-client-ui-model-tag' -ForegroundColor Green
    } catch {
        # 失败则退化为目录副本
        Copy-Item $uiTarget $uiLink -Recurse -Force
        Write-Host '[3/4] junction 失败，已复制目录副本: dsh-client-ui-model-tag' -ForegroundColor Green
    }
} else {
    Write-Host '[3/4] profiles 树已存在 dsh-client-ui-model-tag，跳过' -ForegroundColor Green
}
...
if ($content -match 'ui-model-effort-tag' -and $content -match 'subagent-effort-in-process') {
    Write-Host '[4/4] patch 已包含两个插件行，跳过' -ForegroundColor Green
} else {
    $dir = Split-Path -Parent $patchPath
    if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    Add-Content -Path $patchPath -Value $block -Encoding UTF8
    Write-Host "[4/4] 已追加挂载行: $patchPath" -ForegroundColor Green
}
