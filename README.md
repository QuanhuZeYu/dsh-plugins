...
| 插件 | 包名 | 作用 | 运行时 |
|---|---|---|---|
| ui-model-effort-tag | `@deepseek-ai/dsh-client-ui-model-tag` | 在会话每条助手消息的操作条上显示该消息用的模型与 reasoning-effort 标签 | 浏览器（client 插件，host 侧仅占位） |
...
> **不想动安装树？** effort 插件是自包含的，可放任意目录并把它的 name 改为 `file://<绝对路径>/dsh-subagent-effort-in-process/lib/index.js`（host 半支持 file 形式）。
...
- 重启后打开一个会话：任何一条助手消息旁应出现模型/effort 标签（如 `newapi · deepseek-v4-flash · max`）。
